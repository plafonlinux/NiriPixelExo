...
Rectangle {
    ...
    ListModel { id: filteredSessionModel }

    function buildFilteredSessions() {
        filteredSessionModel.clear()
        if (typeof sessionModel === "undefined") return
        var allowed = ["niri", "gnome"]
        for (var i = 0; i < sessionModel.count; i++) {
            var n = sessionModel.data(sessionModel.index(i, 0), Qt.UserRole + 4)
            var f = sessionModel.data(sessionModel.index(i, 0), Qt.UserRole + 2)
            var d = sessionModel.data(sessionModel.index(i, 0), Qt.DisplayRole)
            var raw = (n || f || d || "").toString().toLowerCase()
            for (var j = 0; j < allowed.length; j++) {
                if (raw.indexOf(allowed[j]) !== -1) { filteredSessionModel.append({ realIndex: i }); break }
            }
        }
        if (filteredSessionModel.count === 0)
            for (var k = 0; k < sessionModel.count; k++)
                filteredSessionModel.append({ realIndex: k })
        var hasIdx = false
        for (var m = 0; m < filteredSessionModel.count; m++)
            if (filteredSessionModel.get(m).realIndex === container.sessionIndex) { hasIdx = true; break }
        if (!hasIdx && filteredSessionModel.count > 0)
            container.sessionIndex = filteredSessionModel.get(0).realIndex
    }
    ...
    Text {
        id: dateText
        text: (new Date()).toLocaleDateString(Qt.locale("ru_RU"), "dddd, d MMMM")
            ...
            ColumnLayout {
                ...
                Rectangle {
                    ...
                    RowLayout {
                        ...
                        Text {
                            text: {
                                if (typeof sessionModel !== "undefined" && filteredSessionModel.count > 0) {
                                    ...
                                    var finalName = n ? n.toString() : (f ? f.toString() : (d ? d.toString() : "Сессия"));
                                    return cleanName(finalName) + (filteredSessionModel.count > 1 ? " ▾" : "");
                                }
                                return "Niri";
                            }
                            color: "white"
    ...
    Popup {
        id: sessionPopup
        width: 260
        height: filteredSessionModel.count > 0 ? Math.min(250, filteredSessionModel.count * 50 + 20) : 100
        x: (parent.width - width) / 2
        ...
        ListView {
            ...
            anchors.fill: parent
            anchors.margins: 10
            model: filteredSessionModel.count > 0 ? filteredSessionModel : null
            spacing: 5
            clip: true
            focus: true
            currentIndex: {
                for (var i = 0; i < filteredSessionModel.count; i++) {
                    if (filteredSessionModel.get(i).realIndex === container.sessionIndex) return i
                }
                return 0
            }
            highlightFollowsCurrentItem: true
            delegate: ItemDelegate {
                ...
                contentItem: RowLayout {
                    ...
                    Text {
                        Layout.fillWidth: true
                        text: {
                            var ri = filteredSessionModel.get(index).realIndex
                            var n_val = sessionModel.data(sessionModel.index(ri, 0), Qt.UserRole + 4);
                            var f_val = sessionModel.data(sessionModel.index(ri, 0), Qt.UserRole + 2);
                ...
                onClicked: {
                    container.sessionIndex = filteredSessionModel.get(index).realIndex;
                    sessionPopup.close();
                }
            }
            Keys.onDownPressed: incrementCurrentIndex()
            Keys.onUpPressed: decrementCurrentIndex()
            Keys.onReturnPressed: { container.sessionIndex = filteredSessionModel.get(currentIndex).realIndex; sessionPopup.close(); }
            Keys.onEnterPressed: { container.sessionIndex = filteredSessionModel.get(currentIndex).realIndex; sessionPopup.close(); }
        }
    }
}
