...
SERVICE = NiriService.get_default() if NiriService and NiriService.get_default().is_available else None
...
class Tasks(widgets.Box):
    ...
    def _window_id(self, window) -> str | None:
        if isinstance(SERVICE, NiriService):
            return window.app_id
        return None

    def _window_matches_app(self, window, app) -> bool:
        wid = self._window_id(window)
        if not wid:
            return False
        if self._is_same_app(app.id, wid):
            return True
        desktop = getattr(app, "_app", None)
        if desktop and hasattr(desktop, "get_startup_wm_class"):
            wm_class = desktop.get_startup_wm_class()
            if wm_class and wm_class.lower() == wid.lower():
                return True
        return False

    def _get_app_from_window(self, window):
        for app in self.applications_service.apps:
            if self._window_matches_app(window, app):
                return app
        return None
    ...
    def _create_app_button(self, app, is_open: bool, is_active: bool = False):
        ...
        if is_open:
            indicator = widgets.Box(css_classes=["app-indicator", bar.side])
            if not bar.vertical:
