...
class ColorCategory(widgets.Box):
    def __init__(self):
        ...
        self.append(
            SwitchRow(
                title="Авто-тёмная",
                description="Автоматически переключать тёмную тему по времени суток.",
        ...
        self.append(
            SettingsRow(
                ...
                description="Время суток для включения тёмной темы.",
                child=[
                    widgets.SpinButton(
                        min=0,
                        max=24,
        ...
        self.append(
            SettingsRow(
                ...
                description="Время суток для отключения тёмной темы.",
                child=[
                    widgets.SpinButton(
                        min=0,
                        max=24,
...
class QuickSelectCategory(widgets.Box):
    def __init__(self, wallpaper_category_ref):
        super().__init__(
            ...
            spacing=0,
            child=[
                CategoryLabel("Быстрый выбор", "image"),
            ],
        )
        ...
        folder_chooser_button = widgets.FileChooserButton(
            label=widgets.Label(label="Выбрать папку"),
        ...
        loading_label = widgets.Label(label="Загрузка обоев...")
    ...
    def _replace_gallery_content(self, new_child, thumbnail_buttons=None):
        ...
        if new_child:
            ...
            self.gallery_content_container.append(
                widgets.Label(
                    label="Обои в директории не найдены.",
                    css_classes=["message"],
                )
            )
...
class FontCategory(widgets.Box):
    def __init__(self):
        ...
        self.append(CategoryLabel("Шрифты", "custom_typography"))
        ...
        self.append(
            SettingsRow(
                title="Шрифт", description="Выберите системный шрифт.", child=[font_button]
            )
        )
